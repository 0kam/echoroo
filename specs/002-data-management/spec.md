...
- **FR-011**: システムは、録音へのノートとタグの追加をサポートしなければならない
...
- **Dataset（データセット）**: 特定の目的のためにグループ化された録音のコレクション。名前、説明、可視性ステータス、インポートステータスを持ち、**サイトに必ず属する**（サイトのないデータセットは作成不可）。以下のメタデータを関連付けることができる：
  ...
  - ゲイン（dB）- 録音時の増幅設定
  - ノート - 内部用メモ
- **Recording（録音）**: メタデータを持つ単一の音声ファイル。ファイル名、ファイルパス、長さ、サンプルレート、チャンネル数、日時、time_expansion（超音波録音の減速再生倍率、デフォルト1.0）を持ち、データセットに属する。複数のクリップとノートを持つことができる
- **Clip（クリップ）**: 録音内の時間セグメント。開始時間、終了時間、親録音への参照を持つ。アノテーションとML予測の分析単位として機能する
- **Note（ノート）**: 録音、クリップ、その他のエンティティに付随するテキスト注釈。内容と作成者を持つ
- **Tag（タグ）**: エンティティの分類とフィルタリングのためのキーと値のペア。録音、クリップ、その他のエンティティに付けることができる
