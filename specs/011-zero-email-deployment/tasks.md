...
- **Done this slice (PR `011-public-invite`, 2026-06-02)**: **US2 PUBLIC INVITE FLOW** (T220/T223) — rewrote `(public)/invite/[token]/+page.svelte` (replacing the old #133 project-scoped flow): resolver → branch on `is_logged_in` → new-user signup (read-only bound email + password≥12 + **client-side TOTP enroll** via `otplib` devDep→dep `generateSecret`/`generateURI` + `qrcode`, submit `totp_enrollment{totp_secret_signed,totp_initial_code}`) OR existing-user accept-confirm. Accept establishes session SERVER-side (HttpOnly cookies, no body token) → hydrate via `authStore.initialize` → `history.replaceState` strips token → land in `/projects/{id}`. `resolveInvitation`+`acceptInvitation` in `web-auth.ts` (T223). Routing already token-leak-safe (`/invite` in NO_REDIRECT, `(public)` no guard — zero infra changes). i18n 16 keys en/ja. code-reviewer: 0 crit/high/med (2 low fixed). **Gate 3 で 3 つの実害バグを連続検出・修正** (内部レビューでは不可視): (1) cold-start refresh latch が accept 後未解除→新規ユーザーが /login に弾かれる→`apiClient.clearRefreshLatch()`、(2) `resolve/accept` の Bearer 付与漏れ→logged-in resolve 401→条件付き Bearer、(3) token-hydration race (hard nav で in-memory token 消失、resolve が fire-and-forget initialize に先行)→`data.isAuthenticated` gate + `apiClient.refreshToken()` await (共有 refreshPromise join で二重 refresh 回避)。**Gate 3 両ブランチ PASS** (新規 signup + client TOTP round-trip → project 着地、既存ユーザー accept → member 追加、console 0、token leak なし)。**残: US7 frontend (D, T640-T643) / Step 12b e2e (E)**。
- **Done this slice (PR `011-us7-frontend`, 2026-06-02)**: **US7 FRONTEND** (T640-T643) — consumes the merged me.py banner/activity API. `lib/api/me.ts` (`listBanners`/`dismissBanner`/`listActivity` via shared `callWebApi` exported from projects.ts → Bearer+CSRF, avoiding the BFF pitfall); `lib/types/me.ts`; `BannerStack.svelte` (TanStack query guarded on `authStore.isAuthenticated`, renders backend `summary` verbatim + View-activity link + ✕ dismiss → mutation invalidates `['me-banners']` on success AND error for anti-enum 404 reconcile, **renders nothing when empty**); mounted in `(app)/+layout.svelte`; `(app)/profile/activity/+page.svelte` (manual `$state` cursor pagination, limit 50, Load-more, renders only `action`+`occurred_at` — never raw `details`/`actor_user_id`); i18n 8 chrome keys en/ja. code-reviewer 0 crit/high/med. **Gate 3 実機 PASS** (実 banner: password-reset/invite-accepted、GET /me/banners 200+Bearer、dismiss 204→reload で消える、activity 50→65 行 cursor pagination 末尾で button 非表示、logged-out=box なし+リクエストなし、console 0)。**follow-up nit (非ブロッカー)**: dismiss POST が 1 クリックで 2 回発火 (両 204 冪等・無害、`$derived(createQuery)` 共通パターン由来の可能性) / (A) new-device login が dev で `auth.login.new_device` banner 行未生成 (login_notification_dispatcher 観察、frontend は他 banner で検証済)。**残: E Step 12b Playwright e2e (T192/T245/T292/T405/T544/T663/T740-T745) — 実装スライスは全完了**。
- **Done this slice (branch `011-step12b-e2e`, 2026-06-02) — STEP 12b PLAYWRIGHT E2E COMPLETE (final slice)**: 8 e2e specs added/un-gated, all driven against the live dev stack with a temporary `TEST_MODE` shared-TOTP enablement (reverted after). **Done**: T192 (`no-email-ui-fresh-deployment` — no verify-email UI on login/profile/dashboard/settings + deleted routes 404), T245 (`single-invitation-flow` — new-user signup w/ scraped client-TOTP enroll, existing-user accept, 409 already-member), T292 (`bulk-invitation-flow` — 5-row table, ≥2 independent redemptions, Copy-all-CSV), T362 (`admin-password-reset` un-gated + run under TEST_MODE; target restored to suite default), T544 (`su-bootstrap` — ownership transfer via API + `pre_transfer_action_summary` via read-only DB query; SC-4), T663 (`banner-stack` — real banner via trusted-device revoke-all, render/dismiss-persist/empty/activity+Load-more), T740 (`sc1-naive-deployer` — login→dashboard→create project, no email gate; SC-1), T745 (`sc6-trusted-device-regression` — FR-011-402 self-revoke trigger; SC-2/SC-3/SC-4 marked done via T292/T362/T544). New shared helpers `tests/e2e/helpers/spec011-auth.ts` (`loginWithSharedTotp`, `redeemInviteAsNewUser`) + `spec011-infra.ts` (console-error tracking, in-browser refresh+CSRF header factory, trusted-device revoke, guarded env-var-driven docker infra: rate-limit reset + UUID-validated audit query). **Adversarial code-review (CRITICAL×4 + IMPORTANT×8) fully remediated**: module-scope serial state (no Node `global`), no `sh -c` shell interpolation, run-specific count assertions (no stale `.first()` false-pass), admin-reset target password restored in `afterAll`, deterministic dismiss waits, bounded loops, paged activity assertion, real ownership-badge assertion, redacted token logging, docker-absent graceful skip. **Full serialized suite (`--workers=1`) green 2×: 28 passed / 1 fixme (24h-TTL path); re-runs leave no account broken; `npm run check` 0 new errors.** Playwright e2e is local-only (NOT in CI per repo config). **T405 (US5 admin 2FA-reset) DEFERRED** — see its task line: no frontend exists + backend is the A-11 delayed-dispatch/M-of-N workflow; needs a dedicated UI+sync-test follow-up. **spec/011 is now functionally COMPLETE** except the deferred T405 follow-up.
...
- [x] T192 [US1] [P] Add Playwright e2e `apps/web/tests/e2e/no-email-ui-fresh-deployment.spec.ts` (US1 AC1-3): bring up with no env vars, complete setup, navigate Profile + Dashboard + Settings, assert no "verify email" / "メール認証" UI anywhere
...
- [x] T220 [US2] [P] Create `apps/web/src/routes/(public)/invite/[token]/+page.svelte` (built under `(public)` NOT `(auth)` — already token-leak-safe; rewrote the old #133 project-scoped page): resolver fetch, signup form vs accept confirmation per `is_logged_in`, bound email read-only, CLIENT-side TOTP enrollment (otplib+qrcode)
- [x] T221 [US2] [P] In `apps/web/src/routes/(app)/projects/[id]/collaborators/+page.svelte` add single-invite form (email + role selector + Issue button + one-shot URL display with Copy)
- [x] T222 [US2] [P] In `apps/web/src/routes/(app)/projects/[id]/collaborators/+page.svelte` extend the invitation listing to show kind=member rows with revoke button (FR-011-108)
- [x] T223 [US2] [P] In `apps/web/src/lib/api/web-auth.ts` (pre-auth transport, NOT auth.ts) add `resolveInvitation` + `acceptInvitation` client methods (both new-user signup and existing-user confirmation payloads; conditional `Authorization: Bearer` so logged-in resolve/accept carries the session)
...
- [x] T245 [US2] [P] Add Playwright e2e `apps/web/tests/e2e/single-invitation-flow.spec.ts` (US2 AC1-6): admin issues, copy URL, second browser opens, signup, lands in project at correct role; repeat for existing-user variant; 409 for already-member
...
- [x] T280 [US3] [P] In `apps/web/src/routes/(app)/projects/[id]/collaborators/+page.svelte` add bulk-mode toggle, newline-separated email paste textarea, single role selector, submit; render result table with "Copy all as CSV" affordance
- [x] T281 [US3] Verify URLs are NOT recoverable after navigation (FR-011-110 / FR-011-113); add a UI warning to that effect
...
- [x] T292 [US3] [P] Add Playwright e2e `apps/web/tests/e2e/bulk-invitation-flow.spec.ts` (US3 AC1-6): paste 5 emails, submit, verify 5 URLs in result table, each redeems independently
...
- [ ] T405 [US5] [P] **DEFERRED (2026-06-02, Step 12b)** — admin 2FA-reset has NO frontend (a UI was never planned in tasks.md and none exists: no button, no client method, no i18n) AND the backend `POST /web-api/v1/admin/users/{id}/reset-2fa` is the Phase 17 A-11 **delayed-dispatch / M-of-N approval** workflow (202-accepted, NOT a synchronous "disable now"). A UI-driven Playwright e2e therefore requires building a new admin feature + a synchronous test path — out of scope for an "add e2e tests" slice. Deferred to a dedicated follow-up (admin 2FA-reset UI + sync dispatch for testability). Backend T400-T404 remain done; SC-6/T745 unaffected. Then add Playwright e2e `apps/web/tests/e2e/admin-2fa-reset.spec.ts` (US5 AC1): admin disables target user's 2FA, target re-enrolls successfully
...
- [x] T544 [US6] [P] Add Playwright e2e `apps/web/tests/e2e/su-bootstrap.spec.ts` (US6 AC1-6): SU creates project for Alice, copies URL, Alice accepts in second browser, ownership transferred and visible in activity view with pre_transfer_action_summary
...
- [x] T640 [US7] [P] Create `apps/web/src/lib/components/BannerStack.svelte`: stacks undismissed banners non-modally (TanStack query, renders nothing when empty/logged-out); each banner dismissable (mutation → invalidate on success AND error for anti-enum 404 reconcile); links to activity view
- [x] T641 [US7] [P] In `apps/web/src/routes/(app)/+layout.svelte` render `BannerStack` (between header and scroll container)
- [x] T642 [US7] [P] Add `apps/web/src/routes/(app)/profile/activity/+page.svelte` (new sibling route) with the full activity history (manual `$state` cursor pagination, limit 50, Load-more)
- [x] T643 [US7] [P] In `apps/web/src/lib/api/me.ts` (NOT auth.ts — exported shared `callWebApi` from projects.ts for Bearer+CSRF) add `listBanners`, `dismissBanner({audit_table, audit_log_id})`, `listActivity({cursor, limit})` client methods
...
- [x] T663 [US7] [P] Add Playwright e2e `apps/web/tests/e2e/banner-stack.spec.ts` (US7 AC1-5): trigger new-device login, API key revoke, email change, 2FA reset; verify banner stack updates, dismiss works, activity view shows full history
...
- [x] T740 [P] **SC-1**: A naive deployer can complete README quickstart §1-§5 without being required to configure SMTP/Resend/Mailpit/DKIM/DNS — manual walkthrough recorded as an end-to-end Playwright test `apps/web/tests/e2e/sc1-naive-deployer.spec.ts` (deferred to Step 12b — frontend Playwright)
- [x] T741 [P] **SC-2**: Lab admin onboards 20 collaborators via one bulk-invitation operation — covered by T292 + a manual lab-scale exercise (deferred to Step 12b — manual exercise)
- [x] T742 [P] **SC-3**: User who forgot password is reset and back to working state under 5 minutes — covered by T362 (deferred to Step 12b — already covered by T362 in Step 5)
- [x] T743 [P] **SC-4**: SU bootstraps a project in a single create-project operation; new owner sees `pre_transfer_action_summary` — covered by T544 (deferred to Step 12b — already covered by T544 in Step 9)
...
- [x] T745 [P] **SC-6**: spec/010 Trusted Device user stories continue to pass (regression check) — re-run spec/010's Playwright suite; additionally assert FR-011-402 revocation triggers fire correctly across the 4 new call-sites (deferred to Step 12b — Playwright)
