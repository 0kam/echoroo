...
- **Activity / banner retention** — `user_banner_dismissals` rows GC'd after 30 days aligned with banner age limit (FR-011-309). `project_audit_log` and `platform_audit_log` themselves are not GC'd by this spec. **Closeout (Step 12)**: tentative answer kept. `services/user_banner.py` declares `DEFAULT_BANNER_MAX_AGE_DAYS: Final[int] = 30`; audit log GC is unchanged by spec/011.
