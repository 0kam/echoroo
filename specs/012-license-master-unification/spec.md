...
- **License rename mid-flight**: The form value carried by the dropdown is the license's stable `id` (per FR-004), so an admin renaming the human-readable label while a user has a value selected does NOT invalidate the in-flight submission. The new project still references the same license row; the next render shows the admin's new name.
- **License history during migration**: Existing rows in `project_license_history` (Phase 7 / FR-086 / FR-087 — see `apps/api/echoroo/models/project.py:271` and `apps/api/echoroo/services/detection_export.py:381`) have enum-typed `old_license` / `new_license` columns. The migration converts these to `VARCHAR(50)` and preserves the existing strings verbatim. Per FR-005a these are snapshots and remain unaffected by subsequent admin renames.
- **Detection export FR-086 column**: The CSV export embeds the project's current license as a string column. After this feature, the export reads `project.license` (a joined `short_name` string) instead of `project.license.value` (legacy enum). The wire shape of the export is unchanged.
...
- **FR-004**: A project MUST reference its license by a stable identifier (the `licenses.id` value) that survives renaming the license. Renaming the human-readable name or short_name in the admin UI MUST NOT orphan any project. The project creation form MUST submit the stable identifier — not the short_name — so that an admin rename completed between the dropdown render and the form submit does not cause the submit to fail.
- **FR-005**: New project creation requires a license (aligning with spec/006 `ProjectCreateRequest.required = [name, visibility, license]`); the field is NOT optional at create time. Pre-existing projects whose legacy `license` enum column was NULL at migration time are preserved as `license_id IS NULL` (the column stays nullable to accommodate them), but no new code path may produce a NULL `license_id` after this feature ships.
- **FR-005a**: License history snapshots (`project_license_history.old_license` / `new_license`) MUST continue to record every license transition. The history columns store the license `short_name` at the time of the change (snapshot, not FK) so that subsequent admin renames do not rewrite historical records — FR-004 "stable identifier" applies forward in time; history is intentionally immutable. The migration converts the existing enum-typed history columns to `VARCHAR(50)` and copies the legacy enum values verbatim.
