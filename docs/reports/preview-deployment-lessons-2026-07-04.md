...
- **§3 fresh-install 小物**: **反映済み**。paraglide は `.gitkeep` ではなく
  named volume + Dockerfile.dev（node 所有でディレクトリ作成）で対処しており、等価。
- **§4 ML device env**: **反映済み**（#168）。`ml_device_env.py` が numpy / TF import 前に
  適用される構成になっている。
- **§5 celery restart ポリシー + watchdog**: **本 PR で反映**（compose の
  restart: unless-stopped 4 サービス追加 + `scripts/worker-watchdog.sh`）。
- **§6 upload session 宙吊り**: ISSUED / UPLOADED には 409 auto-supersede が存在するが、
  VALIDATING / VALIDATED / IMPORTING で宙吊りになったセッションのタイムアウトは
  未実装（既知のフォローアップ）。
