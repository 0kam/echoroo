...
class InferenceBatch(Base):
    ...
    to a set of audio clips. Can be associated with an ML Project
    (legacy) or operate independently with multiple dataset scopes.
    """
    ...
    # Project association - required for standalone batches
    ...
    """The project this batch belongs to (for access control). Required for standalone batches."""
    ...
    """The ML project this inference batch belongs to (legacy, optional)."""
    ...
    """The ML project this batch belongs to (legacy)."""
    ...
    dataset_scopes: orm.Mapped[list["InferenceBatchDatasetScope"]] = orm.relationship(
        "InferenceBatchDatasetScope",
        back_populates="inference_batch",
        default_factory=list,
        cascade="all, delete-orphan",
    ...
    """Dataset scopes for this inference batch."""


class InferenceBatchDatasetScope(Base):
    """Inference Batch Dataset Scope model.

    Defines which datasets to run inference on and which foundation
    model run provides the embeddings for feature extraction.
    """

    __tablename__ = "inference_batch_dataset_scope"
    __table_args__ = (
        UniqueConstraint(
            "inference_batch_id",
            "dataset_id",
    ...
    """The inference batch this scope belongs to."""

    dataset_id: orm.Mapped[int] = orm.mapped_column(
        ForeignKey("dataset.id", ondelete="CASCADE"),
        nullable=False,
    )
    """The dataset to run inference on."""
    ...
    # Optional statistics
    clips_processed: orm.Mapped[int] = orm.mapped_column(
        nullable=False,
        default=0,
    )
    """Number of clips processed in this dataset."""

    positive_count: orm.Mapped[int] = orm.mapped_column(
        nullable=False,
        default=0,
    )
    """Number of positive predictions in this dataset."""
