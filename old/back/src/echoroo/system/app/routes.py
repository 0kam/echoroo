...
from echoroo.plugins import add_plugin_pages, add_plugin_routes, load_plugins
...
def add_routes(app: FastAPI, settings: Settings):
    ...
    from echoroo.routes import get_main_router
    ...
    app.mount(
        "/",
        StaticFiles(packages=["echoroo"], html=True),
        name="static",
    )
