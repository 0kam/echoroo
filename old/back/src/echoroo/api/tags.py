...
class TagAPI(
    ...
    async def create_many(
        ...
        """Create multiple tags, defaulting canonical_name to value."""

        prepped = [
            dict(
                item,
                canonical_name=item.get("canonical_name") or item.get("value"),
            )
            for item in data
        ]
        return await super().create_many(session, prepped)
