...
async def admin_client_factory(  # type: ignore[no-untyped-def]
    ...
    async def _factory(user: User) -> AsyncClient:
        ...
        # ``admin_destructive``). Mint an ``admin_recovery`` token via the
        # canonical issuer so the AND-condition factors are present and
        # the gate passes through to the skip_delay approval logic.
