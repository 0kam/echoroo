...
def _unsupported_reason(file: UploadFile, project_id: UUID, dataset_id: UUID) -> str | None:
    """Why a file cannot be processed after slice 2e, or None when it is staged under its reserved key."""
    if file.received_bytes <= 0:
        return "No staged bytes; upload the file again"
    reserved_prefix = f"recordings/{project_id}/{dataset_id}/{file.id}"
    if not file.object_key.startswith(reserved_prefix):
        return "Legacy upload key; upload the file again"
    return None
        ...
        async with session_factory() as db:
            ...
            for file in files:
                try:
                    ...
                    unsupported_reason = _unsupported_reason(file, project_id, dataset_id)
                    if unsupported_reason is not None:
                        await file_repo.update_status(
                            file.id,
                            UploadFileStatus.INVALID,
                            validation_error=unsupported_reason,
                        )
                        await db.commit()
                        invalid_count += 1
                        continue

                    source = upload_staging.part_path(file.session_id, file.id)
...
def validate_upload_session(self: Any, session_id: str) -> dict[str, Any]:
    ...
    Checks staged bytes, strips GPS metadata, then runs ffprobe to extract
    audio metadata (duration, samplerate, channels, bit_depth). Updates
    per-file and session status in the database.
...
def import_from_upload_session(
    ...
    Publishes each VALID file from POSIX staging to its reserved recordings
    object key, parses datetime from the original filename if configured, and
