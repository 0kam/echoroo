"""Project invitation endpoints (spec/011 + Phase 10 Batch 2).
...
* ``POST   /{project_id}/invitations``                     — issue a
  Member-kind invitation (spec/011 T200, FR-011-101 / FR-011-102).
* ``POST   /{project_id}/invitations/bulk``                — bulk-issue
  (spec/011 Step 8, FR-011-110..115).
* ``POST   /{project_id}/invitations/{invitation_id}/revoke`` — Owner /
  Admin revoke (spec/011 Step 8).
* ``GET    /{project_id}/invitations``                     — unified
  listing (spec/011 T201, FR-011-108).
* ``POST   /{project_id}/invitations/{token}/accept``      — accept invite
  (T511, FR-053 / FR-054).
* ``DELETE /{project_id}/invitations/{token}``             — recipient
  self-decline (T512, FR-107 / FR-101c / FR-054 / FR-055).
...
The membership list / role-change / remove endpoints live in the sibling
:mod:`echoroo.api.web_v1.projects._members` module.
