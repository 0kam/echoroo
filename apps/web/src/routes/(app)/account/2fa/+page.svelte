<script lang="ts">
  /**
   * Two-factor authentication page (Phase 4 redesign).
   *
   * Two modes:
   *
   *  Mode A — enrollment (`?mode=setup` from login flow):
   ...
   *
   *  Mode B — management (default, when not in setup flow):
   *    Show "2FA enabled" notice + admin-only disable hint (FR-072).
   */
  ...
  type Phase =
    | 'loading'
    | 'enroll_form' // QR + code input
    | 'enroll_show_codes' // backup codes screen
    | 'manage'; // already-enrolled management view
  ...
  onMount(async () => {
    const isSetupMode = $page.url.searchParams.get('mode') === 'setup';
...
<div class="mx-auto w-full max-w-2xl px-4 py-10">
  ...
  {:else if phase === 'manage'}
    <h1 class="text-2xl font-extrabold text-stone-900">
      {m.auth_two_factor_management_title()}
    </h1>
    <div class="mt-4 rounded-md border border-success/30 bg-success/10 p-4">
      <p class="text-sm text-stone-800">
        {m.auth_two_factor_management_enabled()}
      </p>
    </div>
    <p class="mt-4 text-sm text-stone-600">
      {m.auth_two_factor_management_admin_only()}
    </p>
  {/if}
</div>
