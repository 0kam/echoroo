...
<button
  ...
  class="inline-flex items-center gap-2 whitespace-nowrap rounded-md px-4 py-2 text-sm font-medium transition-all disabled:cursor-not-allowed disabled:opacity-50
    {variant === 'primary' ? 'border-0 bg-primary-600 text-white hover:bg-primary-700 dark:bg-primary-500 dark:text-stone-50 dark:hover:bg-primary-400' : 'border border-stone-300 bg-stone-100 text-stone-700 hover:bg-stone-200'}"
>
  {#if isDownloading}
