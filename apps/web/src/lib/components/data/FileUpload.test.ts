...
const { fetchActiveUploadSession, createUploadSession } = mockedUploads;
...
vi.mock('$lib/upload/scheduler', () => ({
  ...
  UploadScheduler: class UploadScheduler {
    constructor(private readonly files: Array<{ fileId: string }>, private readonly _opts: unknown, private readonly cb: { onFileFailed: (fileId: string, message: string) => void }) {}
    run() {
